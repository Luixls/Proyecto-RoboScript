...
instrucciones  -> instruccion _ instrucciones {% d => [d[0], ...d[2]] %} | instruccion {% d => [d[0]] %}
instruccion    -> avanzarStmt | girarStmt | esperarStmt | encenderStmt | apagarStmt | siStmt | mientrasStmt
...
siStmt         -> %SI _ condicion _ %ENTONCES _ bloque _ %FIN {% d => ({ type: "Si",       test: d[2],    consequent: d[6] }) %}
mientrasStmt   -> %MIENTRAS _ condicion _ %HACER _ bloque _ %FIN {% d => ({ type: "Mientras", test: d[2], body:       d[6] }) %}

condicion      -> %IDENTIFICADOR _ operador _ ( %NUMERO | %IDENTIFICADOR ) {% d => ({ type:"Condicion", left:d[0].value, operator:d[2], right:d[4].value }) %}
